...
public class GrandMaDialogValidation : DialogValidation
{
	...
	public override bool QuestConditions(VIDE_Assign dialogue) {
		...
		if (VD.isActive) {  //Stuff we check while the dialogue is active
			...
			if (quest?.status == QuestStatus.Done) {
				...
				dialogue.overrideStartNode = AvailableQuestNode;
			} else if (quest?.status == QuestStatus.NonAvailable) {
				dialogue.overrideStartNode = NonAvailableQuestNode;
			}
		}
		return true;
	}
